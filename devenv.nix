...
in
{
  ...
  env = {
    BUNDLE_BUILD__LIBXML___RUBY =
      ...
      + lib.optionals pkgs.stdenv.isDarwin " --with-iconv-dir=${pkgs.libiconv} --with-opt-include=${pkgs.libiconv}/include";

    BUNDLE_BUILD__PG = "${builtins.concatStringsSep " " ruby.gems.pg.buildFlags}";
